namespace iFMIS_BMS.Reports
{
    ...
    public partial class LBP2NewConsolidated : Telerik.Reporting.Report
    {
        ...
        public LBP2NewConsolidated(int OfficeID, int YearOf, int ReportTypeID, int includeLBP1, int pagnoid,long eid,int includezero,int reloadlbp2,int sectoral,int eco)
        {
            InitializeComponent();
                ...
                using (SqlConnection con = new SqlConnection(Common.MyConn()))
                {
                    if (sectoral == 1)
                    {
                        SqlCommand com = new SqlCommand(@"select a.OfficeID,a.OfficeName,a.fundID,b.FundName from tbl_R_BMSOffices as a 
                                                    ...
                                                    where isnull(a.PMISOfficeID,0) != 0 and a.fundid <> 201 and a.officeid <> 37 and a.officeid <> 41 and a.officeid <> 38 and a.officeid <> 43  and a.FundID=101  order by a.SectorID,cast(isnull(a.OrderNo,999999) as integer)", con);
                        con.Open();
                    ...
                    else if (eco == 1)
                    {
                        SqlCommand com = new SqlCommand(@"select a.OfficeID,a.OfficeName,a.fundID,b.FundName from tbl_R_BMSOffices as a 
                                                    LEFT JOIN tbl_R_BMSFunds as b on b.FundCode = a.FundID
                                                    where a.officeid =" + OfficeID + " order by cast(isnull(a.OrderNo,999999) as integer)", con);
                        con.Open();
                        OfficeIDList.Load(com.ExecuteReader());
                    }
                    else
            ...
            if ( OfficeID == 39 || (OfficeID == 38 && eco == 1))
            {
