namespace iFMIS_BMS.Reports
{
    ...
    /// </summary>
    public partial class WFPDFPPTQtr_Revise : Telerik.Reporting.Report
    {
        ...
        //    }
        //}
        public WFPDFPPTQtr_Revise(int? OfficeID=0, int? yearof=0, int? qtr=0,int? mode=0)
        {
            //
            ...
            _wfprev = OleDbHelper.ExecuteDataset(ConfigurationManager.ConnectionStrings["sqldb"].ToString(), CommandType.Text, _sqlQueryrev).Tables[0];
            revisetag = _wfprev.Rows[0][0].ToString();
            if (revisetag != "") // WFP summary revision
            //if (_wfprev.Rows.Count != 0)
            {
                textBox34.Value = "Detailed Financial and Physical Performance Target (Revised: " + revisetag + ")";
            }
            else
            {
            ...
            //{
            //    var data = "";
            //    SqlCommand com = new SqlCommand(@"[sp_BMS_WFPReviseUpdate] " + OfficeID + "," + yearof + "", con);
            //    con.Open();
            ...
            using (SqlConnection con = new SqlConnection(Common.MyConn()))
            {
                con.Close();
                var data = "";
                SqlCommand comrev = new SqlCommand(@"[sp_BMS_WFPReviseUpdate] " + OfficeID + "," + yearof + "", con);
                con.Open();
                data = Convert.ToString(comrev.ExecuteScalar());
