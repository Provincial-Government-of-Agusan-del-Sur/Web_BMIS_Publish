    ...
    public partial class WFPNewExcess : Telerik.Reporting.Report
    {
        public WFPNewExcess(int? year=0, int? office=0, int? project_id=0, int? fundsource=0,string OfficeName="",string ProjectName="", string funddescription="",int? repHistory=0,int? prepby=0,int? printstatus=0,int? projectaip=0,string fundsourcename="",int? accountID=0,string accountname="",string  municipal="",string barangay="",string ooeclass="",int? pgas_loc=0,int? approveprint=0,int? programid=0,int? activityid=0,int? fundid=0)
        {
            ...
            if (repHistory == 0)
            {
                ...
                //textBox84.Value = "03/14/2025 04:10:38 PM";//_dt3.Rows[0]["ServerDate"].ToString();
                //qrcode.Value = "SPBUO6CK65";
                //textBox14.Value = "14-Mar-25";
                //// FOR wfp RECONSTRUCTION
                ...
                _sqlreserve = "exec [sp_bms_WFP_retention_Excess] " + office + "," + accountID + "," + year + " ," + printstatus + "," + Account.UserInfo.eid + "";
             ...
             //   string _sqlQuery2 = "";
               ...
               // }
                ...
                if (approveprint == 1)
                {
                        ...
                        string _sqlwfp = "Select top 1 upper([wfpno]) from ifmis.dbo.tbl_T_BMSWFP_xml_Excess where [qrcode]='" + qrcode.Value + "' and actioncode=1";
            ...
            else //report history
            {
                ...
                table1.DataSource = dt1;
                DataTable _dt3 = new DataTable();
                string _sqlQuery3 = "Select * from ifmis.dbo.tbl_T_BMSWFP_xml_Excess where [wfp_id]="+ repHistory + " and [actioncode]=1";
            ...
            if (fundid == 0)
            {
                //  textBox7.Value = "PROJECT TITLE : " + accountname.Replace("(MOOE)", "").Replace("(CO)", "");
                DataTable dtfundsource = new DataTable();
                string sqlfundsource = "SELECT [Account] + ' ('+b.[FundName] + ')' as fund "+
                                          "FROM[IFMIS].[dbo].[tbl_T_BMSExcessAppropriation] as a left join "+
                                         "[IFMIS].[dbo].[tbl_R_BMSFunds] as b on b.FundFlag=a.FundFlag "+
                                         "where TransactionNo="+ accountID + " and a.ActionCode=1";

                dtfundsource = OleDbHelper.ExecuteDataset(ConfigurationManager.ConnectionStrings["sqldb"].ToString(), CommandType.Text, sqlfundsource).Tables[0];
                if (dtfundsource.Rows.Count > 0)
                {
                    textBox4.Value = "FUND SOURCE (specify) : " + dtfundsource.Rows[0]["fund"].ToString();
                }

                //textBox4.Value = "FUND SOURCE (specify) : " + accountname.Replace("(MOOE)", "").Replace("(CO)", "");
                    //" + fundsourcename + " (Continuing)";
            }
            else
            {
            ...
            string sqlproject = "exec [sp_bms_WFP_Project_Excess] " + office + "," + year + "," + accountID + "";
            dtproject = OleDbHelper.ExecuteDataset(ConfigurationManager.ConnectionStrings["sqldb"].ToString(), CommandType.Text, sqlproject).Tables[0];
            if (dtproject.Rows.Count > 0)
            {
                textBox7.Value = "PROJECT TITLE : " + dtproject.Rows[0]["project"].ToString();
                textBox3.Value = "PROGRAM TITLE : " + dtproject.Rows[0]["program"].ToString();
            }
