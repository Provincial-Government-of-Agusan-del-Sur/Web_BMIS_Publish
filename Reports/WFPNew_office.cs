namespace iFMIS_BMS.Reports
{
    ...
    /// </summary>
    public partial class WFPNew_office : Telerik.Reporting.Report
    {
        public WFPNew_office(int year, int office, int project_id, int fundsource,string OfficeName,string ProjectName, string funddescription,int repHistory,int prepby,int printstatus,int projectaip,string fundsourcename,int accountID,string accountname,string  municipal,string barangay,string ooeclass,int pgas_loc)
        {
            //
            ...
            if (repHistory == 0)
            {
                ...
                if (prepby != 0)
                {
                     _sqlQuery = "select a.EmpNameFull, a.Position,OfficeHead,OfficeDesignation from [pmis].[dbo].[vwMergeAllEmployee] as a left join " +
                ...
                else
                {
                     _sqlQuery = "select a.EmpNameFull, a.Position,OfficeHead,OfficeDesignation from [pmis].[dbo].[vwMergeAllEmployee] as a left join " +
                                    "pmis.dbo.OfficeDescription as b on b.OfficeID = a.Department " +
                                    "where eid = " + prepby + " or a.Department in (Select PMISOfficeID from ifmis.dbo.tbl_R_BMSOffices where OfficeID=" + office + ")";
                }
            ...
            string sqlprogram = "exec sp_bms_WFP_Program " + office + "," + year + "";
