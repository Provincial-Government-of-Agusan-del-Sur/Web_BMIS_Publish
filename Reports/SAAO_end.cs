namespace iFMIS_BMS.Reports
{
    ...
    /// <summary>
    /// Summary description for SAAO_end.
    /// </summary>
    public partial class SAAO_end : Telerik.Reporting.Report
    {
        public SAAO_end(int? OfficeID, int? month_, int? month_To, int? year, int? classtype, int? SAAO_type, int? earmark_type, int? repxmlhistory, string dtetime, int? saaotag)
        {
            //
            ...
            if (earmark_type == 3)
            {
                textBox1.Value = "Note: Obligations include earmarked amount!";
            }

            if (repxmlhistory != 0)
            {
                DataTable _dt2 = new DataTable();
                string _sqlQuery2 = "Select [username],[dateprinted],[preparedby],[preparedbyposition],[notedby],[notedbyposition],[qrcode] from [IFMIS].[dbo].[tbl_T_BMSReportXML] where "+
                    "[dateprinted] in (Select top 1 a.dateprinted from [IFMIS].[dbo].[tbl_T_BMSReportXML] as a where rprtid= "+ @repxmlhistory + " and actioncode=1) and office="+ OfficeID + "";
            ...
            using (SqlConnection con = new SqlConnection(Common.MyConn()))
            {
                //SqlCommand com = new SqlCommand(@"IFMIS.dbo.sp_Monthly_SAAO_REPORT_Total '" + month_ + "','" + month_To + "','" + year + "','" + OfficeID + "','" + SAAO_type + "'," + earmark_type + "," + Account.UserInfo.eid + ",'','','','','','" + dtetime + "',1", con);
                SqlCommand com = new SqlCommand(@"IFMIS.dbo.sp_Monthly_SAAO_REPORT_Total '" + month_ + "','" + month_To + "','" + year + "','" + OfficeID + "','" + SAAO_type + "'," + earmark_type + "," + repxmlhistory + ",'" + dtetime + "','" + textBox35.Value + "','" + textBox44.Value + "','JAVE NHORIEL N. BORDAJE, CPA','Provincial Budget Officer'", con);
                com.CommandTimeout = 0;
                con.Open();
