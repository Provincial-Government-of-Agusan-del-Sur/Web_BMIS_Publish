    ...
    public partial class WFPNew : Telerik.Reporting.Report
    {
        public WFPNew(int? year=0, int? office=0, int? project_id=0, int? fundsource=0,string OfficeName="",string ProjectName="", string funddescription="",int? repHistory=0,int? prepby=0,int? printstatus=0,int? projectaip=0,string fundsourcename="",int? accountID=0,string accountname="",string  municipal="",string barangay="",string ooeclass="",int? pgas_loc=0,int? approveprint=0,int? programid=0,int? activityid=0,int? fundid=0,int? subppaid=0,int? specid=0,int? issupplemetalid = 0)
        {
            ...
            if (repHistory == 0)
            {
                ...
                //textBox84.Value = "10/30/2025 10:19:19 AM";//_dt3.Rows[0]["ServerDate"].ToString();
                //qrcode.Value = "0GR71AG8";
                //textBox34.Value = "WFP-24-4427";
                //////// FOR wfp RECONSTRUCTION
                ...
                if (dtrevsion.Rows.Count > 0 && accountID != 2861)
                {
                    if (Convert.ToInt32(dtrevsion.Rows[0]["wfp_id"].ToString()) != 0)
                    {
                        textBox1.Value = "WORK AND FINANCIAL PLAN C.Y. " + year + "" + status + " (Revised From " + dtrevsion.Rows[0]["wfpno"].ToString() + ") "+ supplemental + "";
                    }
                    else
                    {
                        textBox1.Value = "WORK AND FINANCIAL PLAN C.Y. " + year + "" + status + " " + supplemental + "";
                    }
                }
                else
                {
                ...
                if (fundid == 0)
                {
                    ...
                    {
                        _sqlreserve = "exec [sp_bms_WFP_retention_specificactivity] " + office + "," + accountID + "," + year + " ," + printstatus + "," + Account.UserInfo.eid + ","+ specid +"";
                    ...
                    {
                        //_sqlreserve = "exec sp_bms_WFP_retention_after " + office + "," + accountID + "," + year + " ," + printstatus + "," + Account.UserInfo.eid + ",0";
                        _sqlreserve = "exec sp_bms_WFP_retention_after " + office + "," + accountID + "," + year + " ," + printstatus + "," + Account.UserInfo.eid + ",0,"+ issupplemetalid + "";
                        
                        _dtreserve = OleDbHelper.ExecuteDataset(ConfigurationManager.ConnectionStrings["sqldb"].ToString(), CommandType.Text, _sqlreserve).Tables[0];
                        if (_dtreserve.Rows.Count > 0)
                        {
                            if (_dtreserve.Rows[0][7].ToString() == "1" && _dtreserve.Rows[0][1].ToString() != "0.00")
                            {
                                textBox132.Value = _dtreserve.Rows[0][0].ToString();
                                textBox157.Value = _dtreserve.Rows[0][1].ToString();
                                // textBox127.Width='2.34in';
                ...
                {
                    if (specid != 0) {
                        var query = @"exec ifmis.dbo.[sp_bms_WFP_report_grouping_peractivity_specific] " + office + "," + year + "," + Account.UserInfo.eid + ",'" + qrcode.Value + "'," + prepby + ",'CYNTHIA P. LUMANTA, CE, EnP','Acting Provincial Government Department Head','JAVE NHORIEL N. BORDAJE, CPA','Provincial Budget Officer','SANTIAGO B. CANE, JR.','Provincial Governor','" + textBox84.Value + "'," + printstatus + "," + accountID + ",'" + municipal + "','" + barangay + "'," + pgas_loc + "," + approveprint + "," + activityid + ","+ specid + "";
                    ...
                    {
                        if (activityid != 0 && accountID != 2861 && subppaid == 0)
                        {
                        ...
                        else if (subppaid != 0) //per sub-ppa
                        {
                            var query = @"exec ifmis.dbo.[sp_bms_WFP_report_persubppa] " + office + "," + year + "," + Account.UserInfo.eid + ",'" + qrcode.Value + "'," + prepby + ",'CYNTHIA P. LUMANTA, CE, EnP','Acting Provincial Government Department Head','JAVE NHORIEL N. BORDAJE, CPA','Provincial Budget Officer','SANTIAGO B. CANE, JR.','Provincial Governor','" + textBox84.Value + "'," + printstatus + "," + accountID + ",'" + municipal + "','" + barangay + "'," + pgas_loc + "," + approveprint + "," + activityid + "," + subppaid + "";
                            DataTable dt1 = new DataTable();
                            using (SqlConnection con = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["adodb"].ConnectionString))
                            {
                                SqlCommand com = new SqlCommand(query, con);
                ...
                if (fundid == 0)
                {
                    ...
                    {
                        _sqlreserve = "exec sp_bms_WFP_retention_after " + office + "," + accountID + "," + year + " ," + printstatus + "," + Account.UserInfo.eid + ",1,"+ issupplemetalid + "";
                        _dtreserve = OleDbHelper.ExecuteDataset(ConfigurationManager.ConnectionStrings["sqldb"].ToString(), CommandType.Text, _sqlreserve).Tables[0];
                        if (_dtreserve.Rows.Count > 0)
                        {
                            textBox132.Value = _dtreserve.Rows[0][0].ToString();
            ...
            {
                textBox4.Value = "FUND SOURCE (specify) : Trust Fund - " + accountname + "";
            }
