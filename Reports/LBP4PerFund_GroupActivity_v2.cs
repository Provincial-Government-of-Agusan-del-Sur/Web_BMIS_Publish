namespace iFMIS_BMS.Reports
{
    ...
    /// </summary>
    public partial class LBP4PerFund_GroupActivity_v2 : Telerik.Reporting.Report
    {
        //
        ...
        public LBP4PerFund_GroupActivity_v2(int OfficeID, int YearOf, int isNonOffice,int reloadlbp4)
        {
                ...
                while (reader.Read())
                {
                    if (Convert.ToInt16(reader.GetValue(0).ToString()) >= 4)
                    {
                        using (SqlConnection con = new SqlConnection(Common.MyConn()))
                        {
                            ...
                            //SqlCommand com2 = new SqlCommand(@"exec sp_BMS_LBP4_LBP2_consolidate " + OfficeID + "," + YearOf + "," + reloadlbp4 + "", con); //consolidation - old
                            //SqlCommand com2 = new SqlCommand(@"exec sp_BMS_LBP4_LBP2_consolidate_perprogram " + OfficeID + "," + YearOf + "," + reloadlbp4 + "", con); //consolidation - new
                            SqlCommand com2 = new SqlCommand(@"exec sp_BMS_LBP4_LBP2_consolidate_peractivity " + OfficeID + "," + YearOf + "," + reloadlbp4 + "", con); //consolidation - new
                    ...
                    else
                    {
                        using (SqlConnection con = new SqlConnection(Common.MyConn()))
                        {
                            SqlCommand com2 = new SqlCommand(@"exec sp_BMS_LBP4 9999,9999,3", con);
                            con.Open();
                            dt.Load(com2.ExecuteReader());
                        }             
            ...
            {
                SqlCommand com = new SqlCommand(@" exec [sp_BMS_ReportLog] 14," + Account.UserInfo.eid + ",'LBPxF4','" + barcode1.Value + "'", conUser);
                conUser.Open();
            ...
            {
                SqlCommand com = new SqlCommand(@"exec [sp_BMS_LBPReport_Update] " + YearOf + "," + OfficeID + ", 5,'" + barcode1.Value + "'", conUpdateRep);
                conUpdateRep.Open();
