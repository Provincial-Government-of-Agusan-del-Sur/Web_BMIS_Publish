namespace Public_TAMS.Reports.Design
{
    ...
    /// </summary>
    public partial class LBP3New_JO : Telerik.Reporting.Report
    {
        ...
        public LBP3New_JO(int OfficeID, int YearOf, int isAllOffice, ref int StartCountRow)
        {
            InitializeComponent();
            ...
            dt.Columns.Add("FundType");//9DivisionName
            dt.Columns.Add("Num");//8
            dt.Columns.Add("Charges");//9

            using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["adodb"].ConnectionString))
            {
                SqlCommand com = new SqlCommand($"select * from nald_vw_jo_plantilla where office_id = @office_id and YearOf = @YearOf order by current_salary desc,position asc", con);
                con.Open();
            ...
            Report.ReportParameters["ReportName"].Value = BMSOfficeID == 1 || BMSOfficeID == 37 || BMSOfficeID == 38 || BMSOfficeID == 41 ? "Job Order of Personnel CY <span style='text-decoration: underline'>" + YearOf + "</span>" : "";
