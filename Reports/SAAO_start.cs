namespace iFMIS_BMS.Reports
{
    ...
    public partial class SAAO_start : Telerik.Reporting.Report
    {
        public SAAO_start(int? OfficeID, int? month_, int? month_To, int? year, int? classtype, int? SAAO_type,int? earmark_type,int? repxmlhistory,string dtetime,int? saaotag)
        {
            InitializeComponent();
            ...
            if (repxmlhistory != 0)
            {
                ...
                string _sqlQuery2 = "Select [username],[dateprinted],[preparedby],[preparedbyposition],[notedby],[notedbyposition],[qrcode],monthof,yearof from [IFMIS].[dbo].[tbl_T_BMSReportXML] where [rprtid] = " + repxmlhistory + "";
                ...
                using (SqlConnection con = new SqlConnection(Common.MyConn()))
                {
                    ...
                    SqlCommand com2 = new SqlCommand(@"SELECT CONCAT((SELECT DATENAME(month, DATEADD(month, '" + month_ + "'-1, CAST('2008-01-01' AS datetime)))), " +
                                                  " ' - ', (SELECT DATENAME(month, DATEADD(month, '" + _dt2.Rows[0][7].ToString() + "'-1, CAST('2008-01-01' AS datetime)))))", con);
                    if (month_To == 1)
                    {
                        TXT_for_the.Value = "For the period of January " + _dt2.Rows[0][8].ToString();
                    }
                    else
                    {
                        TXT_for_the.Value = "For the period of " + com2.ExecuteScalar().ToString() + " " + year;
                    }
                }
            ...
            else
            {
                ...
                using (SqlConnection con = new SqlConnection(Common.MyConn()))
                {
                    ...
                    {
                        TXT_for_the.Value = "For the period of " + com.ExecuteScalar().ToString() + " " + year;
                    }
                }
