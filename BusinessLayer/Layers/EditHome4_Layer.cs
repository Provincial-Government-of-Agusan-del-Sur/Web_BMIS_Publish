...
{
    public class EditHome4_Layer
    {


        public IEnumerable<AccountsModel> Programss(int? prog_ID, int? off_ID, int? propYear)
        {
            List<AccountsModel> prog = new List<AccountsModel>();
            using (SqlConnection con = new SqlConnection(Common.MyConn()))
            {
                SqlCommand com = new SqlCommand(@"SELECT DISTINCT e.AccountID,  e.AccountName,  f.OOEAbrevation,a.ProposalYear,a.ProposalAllotedAmount,e.AccountYear,e.ProgramID,b.AccountCode FROM dbo.tbl_T_BMSBudgetProposal AS a
                                                    ...
                                                    + "-1 and a.ProposalActionCode = '1' and a.ProposalStatusCommittee='1' and e.AccountYear='" + propYear + "' and (e.AccountID  NOT in(select AccountID from tbl_T_BMSBudgetProposal where ProposalYear = '" + propYear + "' ))", con);
                con.Open();
                ...
                while (reader.Read())
                {
                    ...
                    emp.AccountCode = reader.GetInt32(7);
                    prog.Add(emp);
                }
            }
            return prog;
        ...
        public void UpdateAccountHome4(IEnumerable<AccountsModel> Accounts)
        {
            using (SqlConnection con = new SqlConnection(Common.MyConn()))
            {
                con.Open();
